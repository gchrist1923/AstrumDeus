  ...
  return (
    <form action={savePlayer} className="flex max-w-2xl flex-col gap-6">
      ...
      <Field id="ign" label="IGN">
        <input id="ign" name="ign" required defaultValue={player?.ign} className={KELAS_KONTROL} />
      </Field>
      <Field id="slug" label="Slug">
        <input id="slug" name="slug" required defaultValue={player?.slug} className={KELAS_KONTROL} />
      </Field>
      <Field id="realName" label="Nama asli">
        <input id="realName" name="realName" defaultValue={player?.realName} className={KELAS_KONTROL} />
      </Field>
      <Field id="role" label="Role">
        <select id="role" name="role" defaultValue={player?.role ?? 'IGL'} className={KELAS_KONTROL}>
          {ROLES.map((role) => (
      ...
      <Field id="leftAt" label="Keluar">
        <input id="leftAt" name="leftAt" type="date" defaultValue={player?.leftAt ? toDateInput(player.leftAt) : ''} className={KELAS_KONTROL} />
      </Field>
      <label className="flex min-h-11 items-center gap-3">
        <input type="checkbox" name="isActive" defaultChecked={player?.isActive ?? true} className="size-5 accent-accent" />
      ...
      <Field id="socials" label="Sosial" hint="Satu baris: Label|https://...">
        <textarea id="socials" name="socials" rows={3} defaultValue={socials} className={KELAS_KONTROL} />
      </Field>
